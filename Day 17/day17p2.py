...
def ScanForRepeatingPattern(nro_roca):
  nro_roca_5 = nro_roca % 5
  global current_maxY
  global snaps
  haysalto = False
  snap = ""
  cambio_en_altura_por_ciclo = None
  cambio_en_cant_rocas = None
  
  if(current_maxY < 21):
    return False,None,None
  for y in range(current_maxY, current_maxY - 20, -1):
    for x in range(0,7):
      pair = (x,y)
      value = TryGetValue(pair)
      snap += value
  if (snap, len(line), nro_roca_5) in snaps:
    alto, iter = snaps[(snap, len(line), nro_roca_5)]
    cambio_en_altura_por_ciclo = current_maxY-alto
    cambio_en_cant_rocas = nro_roca-iter
    haysalto = True
    #print("-----Ciclo wiiiii-----", cambio_en_altura_por_ciclo, cambio_en_cant_rocas)
  snaps[(snap, len(line), nro_roca_5)] = (current_maxY, nro_roca)
  return haysalto, cambio_en_altura_por_ciclo, cambio_en_cant_rocas
...
print("====== PART 2 ======")
while x <= howManyRocks:
  if not did_jump:
    can_jump, altitude_change, rock_n_change =  ScanForRepeatingPattern(x)
  if not did_jump and can_jump:
    print(" ------Haciendo Salto!------ ")
    rocas_restantes = howManyRocks - x
    posible_jumps = rocas_restantes//rock_n_change
    x+= posible_jumps * rock_n_change
    dif_en_maxY = posible_jumps * altitude_change
    delta= dif_en_maxY
    did_jump=True
  if(x % 1000 == 0):
  ...
  SimulateBlock(x)
  x+=1
print("The tower is", current_maxY+delta, "units tall")
# We draw the tower
#PrintTower()
