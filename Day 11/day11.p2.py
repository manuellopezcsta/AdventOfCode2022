...
# We find the Minimo comun multiplo.
for monkey in monkey_array:
  min_comun_multiplo = lcm(min_comun_multiplo, monkey.test)
...
def resolve_monkey_round(monkey):
  ...
  for x in list(monkey.items):
    ...
    new = 0
    old = x
    new = int(math.trunc(eval(monkey.op)))
    new = new % min_comun_multiplo
